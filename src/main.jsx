...
function LoginScreen({onLogin}){
  ...
  const sendPasswordReset = async () => {
    ...
    try {
      ...
      const rawMessage = String(err?.message || err?.error_description || err?.name || 'sem detalhe retornado');
      const errorMessage = rawMessage.toLowerCase();
      const errorDetail = [rawMessage, err?.status ? `status ${err.status}` : '', err?.code ? `codigo ${err.code}` : ''].filter(Boolean).join(' · ');
      if(errorMessage.includes('redirect') || errorMessage.includes('not allowed') || errorMessage.includes('url')){
        ...
        setError(`Não foi possível enviar o link de redefinição agora. Detalhe Supabase: ${errorDetail}.`);
      }
    } finally {
...
function UXStyle(){
  return <style>{`
    ...
    .formGrid label{align-self:end!important;width:100%!important;}
    .formGrid input,.formGrid select{width:100%!important;height:48px!important;min-height:48px!important;margin:0!important;line-height:1.2!important;}
    .formGrid select{-webkit-appearance:none!important;appearance:none!important;background-image:linear-gradient(45deg,transparent 50%,#64748b 50%),linear-gradient(135deg,#64748b 50%,transparent 50%)!important;background-position:calc(100% - 18px) 50%,calc(100% - 12px) 50%!important;background-size:6px 6px,6px 6px!important;background-repeat:no-repeat!important;padding-right:34px!important;}
    ...
    const checkMeetings = () => {
      ...
      const nextMeeting = safeArray(activities)
        .filter(activity =>
          ...
          dateOnlyFromCrmValue(activity.dueDate) === today() &&
          activity.dueTime &&
          activity.meetingLink &&
    ...
    <section className="grid2 compact">
      ...
      <Panel title={`Contatos sem empresa (${contactsWithoutCompany.length})`}>
        <DashboardTable headers={['Contato','Cargo','Ações']}>
          {contactsWithoutCompany.length ? contactsWithoutCompany.map(contact=><tr key={contact.id} onClick={()=>setSelectedContactId?.(contact.id)} style={{cursor:'pointer'}}><td><b>{contact.name}</b><span>{contact.email || [contact.phone,contact.whatsapp].filter(Boolean).join(' / ') || '-'}</span></td><td>{contact.role || '-'}</td><td><button className="mini" onClick={event=>{event.stopPropagation();setSelectedContactId?.(contact.id)}}><Edit3 size={15}/>Corrigir</button></td></tr>) : <tr><td>Todos os contatos possuem empresa</td><td>-</td><td>-</td></tr>}
        </DashboardTable>
      </Panel>
    </section>
  ...
  return <>
    {canWrite && <Panel title={draft.id ? 'Editar documento' : 'Adicionar documentos'}>
      <div className="formGrid modalGrid">
        ...
        <Input label="Link compartilhado do Dropbox" field="url" form={draft} setForm={setDraft} type="url"/>
        <Textarea label="Observações" field="notes" form={draft} setForm={setDraft}/>
        <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
          <button className="saveBtn" onClick={saveManualDocument}><Link2 size={16}/>{draft.id ? 'Salvar alterações' : 'Adicionar Link do Arquivo'}</button>
...
function Contacts({contacts,setContacts,companies,query,setSelectedContactId,canWrite}){
  ...
  return <>{canWrite && <Panel title="Novo contato"><div className="formGrid"><Input label="Nome" field="name" form={form} setForm={setForm}/><Select label="Empresa" field="companyId" form={form} setForm={setForm} options={safeArray(companies).map(c=>[c.id,c.name])}/><Input label="Cargo" field="role" form={form} setForm={setForm}/><Input label="E-mail" field="email" form={form} setForm={setForm}/><Input label="Telefone 1" field="phone" form={form} setForm={setForm}/><Input label="Telefone 2" field="whatsapp" form={form} setForm={setForm}/><button className="saveBtn" onClick={add}><Plus size={16}/>Salvar contato</button></div></Panel>}<Panel title={`Contatos (${list.length})`}><Table headers={['Contato','Empresa','Cargo','E-mail','Telefones','Tipo','Ações']}>{list.map(c=><tr key={c.id} onClick={()=>setSelectedContactId(c.id)} style={{cursor:'pointer'}}><td><b>{c.name}</b></td><td>{byId(companies,c.companyId)?.name}</td><td>{c.role}</td><td>{c.email}</td><td>{[c.phone,c.whatsapp].filter(Boolean).join(' / ')}</td><td>{c.type}</td><td><div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}><button className="mini" onClick={(e)=>{e.stopPropagation(); setSelectedContactId(c.id)}}><Edit3 size={15}/>Abrir</button>{canWrite && <button className="mini" onClick={(e)=>{e.stopPropagation(); removeContact(c)}}><Trash2 size={15}/>Excluir</button>}</div></td></tr>)}</Table></Panel></>;
}
  ...
  return <div className="modalBackdrop"><div className="modal"><div className="modalHead"><div><h2>{company.name}</h2><span>Cadastro da empresa</span></div><button className="iconBtn" onClick={onClose}><X/></button></div>
    ...
    <Panel title={`Contatos vinculados (${linkedContacts.length})`}>
      <DashboardTable headers={['Contato','Cargo','E-mail','Telefone / WhatsApp','Tipo','Ações']}>
        {linkedContacts.length ? linkedContacts.map(c=><tr key={c.id} onClick={()=>openContact(c.id)} style={{cursor:'pointer'}}><td><b>{c.name}</b><span>{c.notes}</span></td><td>{c.role || '-'}</td><td>{c.email || '-'}</td><td>{[c.phone,c.whatsapp].filter(Boolean).join(' / ') || '-'}</td><td>{c.type || '-'}</td><td><button className="mini" onClick={(e)=>{e.stopPropagation(); openContact(c.id)}}><Edit3 size={15}/>Abrir contato</button></td></tr>) : <tr><td>Nenhum contato vinculado</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td></tr>}
      </DashboardTable>
    </Panel>
  </div></div>;
}
