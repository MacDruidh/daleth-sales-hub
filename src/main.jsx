...
function Deals({deals,setDeals,companies,contacts,stages,setSelectedDealId,query}){
  ...
  return <>
    ...
    <Panel title="Oportunidades"><Table headers={['Oportunidade','Empresa','Produto','Valor','Etapa','Responsável','Ações']}>{list.map(d=><tr key={d.id}><td><b>{d.title}</b><span>{d.nextStep}</span></td><td>{companies.find(c=>c.id===Number(d.companyId))?.name}</td><td>{d.product}</td><td>{money(d.value)}</td><td><span className="pill">{d.stage}</span></td><td>{d.owner}</td><td><button className="mini" onClick={()=>setSelectedDealId(d.id)}><Edit3 size={15}/>Abrir</button></td></tr>)}</Table></Panel>
  </>;
}
...
function Activities({activities,setActivities,deals,query}){
  ...
  return <Panel title="Atividades e follow-ups"><Table headers={['Status','Tipo','Atividade','Oportunidade','Data','Responsável']}>{list.map(a=><tr key={a.id}><td><button className="mini" onClick={()=>toggle(a.id)}>{a.status}</button></td><td>{a.type}</td><td><b>{a.title}</b><span>{a.notes}</span></td><td>{deals.find(d=>d.id===a.dealId)?.title}</td><td>{a.dueDate}</td><td>{a.owner}</td></tr>)}</Table></Panel>;
}
